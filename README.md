...
- Frequency-aware seasonal period detection — infers the correct seasonal period from the pandas date index (hourly=24, daily=7, weekly=52, monthly=12, quarterly=4); uses a length heuristic only when the frequency cannot be determined
...
- Forecast Health Score (0–100) — three-component score: model accuracy vs baseline (40 pts), confidence band calibration against 80% target coverage (30 pts), and data quality including length and missing-value rate (30 pts)
...
- AI anomaly explanation — on-demand per-anomaly explanation; passes date, value, z-score, expected value, and dataset context to the LLM; responses are cached client-side for the session
...
- PDF export — generates a structured PDF briefing via ReportLab with dataset label, quality report, forecast summary, anomaly list, and key findings
...
Python 3.12 (pinned in runtime.txt for Render deployment).
...
4. Click Run Analysis. Results appear in 5–15 seconds.
...
Click Export PDF in the top header to download a formatted briefing containing the dataset summary, quality report, forecast narrative, and key findings.
...
5. run_forecast — detects seasonal period from the pandas frequency string; fits up to three ETS variants and selects by AIC; bootstraps 500 residual paths for confidence bands; clips to ±5σ; runs additive decomposition
...
The 0–100 score combines three measurable components: model accuracy relative to the stated forecast horizon (40 points, based on MAPE against a 20% ceiling), confidence band calibration against the target 80% coverage rate (30 points, penalised for both over- and under-coverage), and data quality including series length and missing-value rate (30 points). The score gives judges and users a single interpretable signal about whether a given forecast should be trusted.
